...
func (d *module) CreateModule(ctx context.Context, namespace, provider, name string) error {
	moduleRootPath := fmt.Sprintf("%s/modules/%s/%s/%s", localRootPath, namespace, provider, name)
...
func (d *module) CreateVersion(ctx context.Context, namespace, provider, name, version string) (*driver.CreateModuleVersionResult, error) {
	versionRootPath := fmt.Sprintf("%s/modules/%s/%s/%s/versions/%s", localRootPath, namespace, provider, name, version)
...
func (d *module) GetModule(ctx context.Context, namespace, provider, name, version string) (*os.File, error) {
	packagePath := fmt.Sprintf("%s/modules/%s/%s/%s/versions/%s/terraform-%s-%s-%s.tar.gz", localRootPath, namespace, provider, name, version, provider, name, version)
...
func (d *module) SavePackage(ctx context.Context, namespace, provider, name, version string, b io.Reader) error {
	pkgPath := fmt.Sprintf("%s/modules/%s/%s/%s/versions/%s/terraform-%s-%s-%s.tar.gz", localRootPath, namespace, provider, name, version, provider, name, version)
